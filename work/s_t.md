...
select s.sno 学号, s.Sname 姓名, p.选修门数, p.总学分
from
(select t.Sno 学号, SUM(t.Ccredit) 总学分,COUNT(Cno) 选修门数
from(
	select Sno,c.Cno,Grade,Ccredit
	from SC, Course c
	where sc.Cno=c.Cno and Grade>=60
) t, Student s
where s.Sno=t.Sno
group by t.Sno) p, Student s
where p.学号=s.Sno
